...
{
    public class AVLBST<T>
    {
        public AVLBST(Comparison<T> comparison)
        {
        ...
        public bool RemoveNode(T data)
        {
            ...
            while (true)
            {
                ...
                {
                    return RemoveCurrentNode(currentNode, isLeft);
                }
            }
        }

        public int RemoveRangeClose(T min, T max)
        {
            if (root == null)
            {
                return 0;
            ...
            while (true)
            {
                var compareMinResult = CompareNode(min, currentNode.data);
                var compareMaxResult = CompareNode(max, currentNode.data);
                ...
                {
                    removedNumber += currentNode.weight;
                    RemoveCurrentNode(currentNode, isLeft, true);
                    currentNode = root;
                }
            }
        }
        ...
        private void MinusParentChildrenNum(AVLTreeNode<T> node)
        {
            var parent = node.parent;
            while (parent != null)
            {
        ...
        private void MinusParentChildrenNum(AVLTreeNode<T> node, AVLTreeNode<T> endNode, int weight)
        {
            var parent = node.parent;
        ...
        private AVLTreeNode<T> GetNextInOrderNode(AVLTreeNode<T> node)
        {
            if (node.rightChild != null)
            {
        ...
        private void Rotate(AVLTreeNode<T> parent, bool isChildLeft, bool isGrandchildLeft)
        {
            try
            {
        ...
        private void LeftRotate(AVLTreeNode<T> parent)
        {
        ...
        private void RightRotate(AVLTreeNode<T> parent)
        {
        ...
        private void UpdateChildrenNumberByDirectChildren(AVLTreeNode<T> parent)
        {
        ...
        private int CheckChildrenNum(AVLTreeNode<T> node)
        {
        ...
        //O(N)
        private List<T> InOrderTraverse(AVLTreeNode<T> node)
        {
        ...
        //O(N)
        private List<T> InOrderTraverseFullInfo(AVLTreeNode<T> node)
        {
            var result = new List<T>();
            if (node.leftChild != null)
            {
